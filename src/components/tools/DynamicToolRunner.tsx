    ...
    <div className="space-y-6">
      <div className="grid lg:grid-cols-[0.85fr_1.15fr] gap-5">
        <div className={`${panelClass} space-y-5`}>
          ...
          {error && (
            <div className="flex gap-2 rounded-xl border border-white/15 bg-white/[0.03] p-3 text-sm text-white/60">
  ...
  return (
    <div className="space-y-5">
      <div className={`${panelClass} space-y-4`}>
        ...
        {error && <div className="rounded-xl border border-white/15 bg-white/[0.03] p-3 text-sm text-white/60">{error}</div>}
      </div>
...
export default function DynamicToolRunner({ tool }: { tool: PublicTool }) {
  ...
  switch (tool.slug) {
    ...
    case "favicon-generator":
      ...
      break
    default:
      content = <GenericToolRunner tool={tool} />
  }
