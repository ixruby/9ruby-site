...
const products = [
  ...
  },
  {
    ...
    desc: "Drag-and-drop website builder direction for client sites and internal launch systems. The standalone app is retired; the useful builder concept now lives inside official 9Ruby products and managed builds.",
    icon: <Blocks size={20} strokeWidth={1.5} />,
    badge: "Consolidated",
    badgeStyle: "border border-white/[0.12] text-white/40",
    href: "/products",
    features: ["Visual editing", "Any stack", "Templates", "Managed setup"],
  },
  {
    name: "Novavox",
...
const CATEGORY_PALETTE: Record<string, { bg: string; accent: string }> = {
  Core:             { bg: "linear-gradient(135deg,#0d0d1a,#1a0a2e)", accent: "#fff" },
...
function ProjectVisual({ name, category, status }: { name: string; category: string; status: string }) {
  const pal = CATEGORY_PALETTE[category] ?? { bg: "linear-gradient(135deg,#0d0d0d,#1a1a1a)", accent: "#fff" }
        ...
        <div className="relative max-w-[1200px] mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-28">
            {products.map((p) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 mt-auto mb-6">
                  {p.features.map((f) => (
                    <span key={f} className="text-xs flex items-center gap-1.5" style={{ color: "rgba(255,255,255,0.35)" }}>
                      <span className="w-1 h-1 rounded-full bg-[#fff]/50 shrink-0" />
                      {f}
                    </span>
                  ))}
                </div>
                ...
                {/* Link */}
                <div className="flex items-center gap-2 text-sm group-hover:text-[#fff] group-hover:gap-3 transition-all" style={{ color: "rgba(255,255,255,0.5)" }}>
          ...
          <div className="mb-28">
            <div className="mb-12 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <span className="text-[11px] font-semibold tracking-[0.12em] uppercase mb-4 block" style={{ color: "#fff" }}>Vercel Registry</span>
              ...
              {vercelBrandProjects.map((project) => {
                ...
                const content = (
                  <>
                    ...
                    <div className="flex items-center gap-2 text-sm group-hover:text-[#fff] transition-colors" style={{ color: "rgba(255,255,255,0.5)" }}>
          ...
          <div className="mb-28">
            <div className="mb-12">
              <span className="text-[11px] font-semibold tracking-[0.12em] uppercase mb-4 block" style={{ color: "#fff" }}>Infrastructure</span>
              <h2 className="text-3xl md:text-4xl font-sans tracking-tighter mb-3" style={{ color: "#fff" }}>
                Live across the web
              </h2>
