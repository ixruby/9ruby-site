...
function CellValue({ value }: { value: string | boolean }) {
  if (typeof value === "boolean") {
    return value
      ? <CheckCircle2 size={14} style={{ color: "#fff", margin: "0 auto", display: "block" }} />
  ...
  return (
    <main id="main-content" style={{ background: "#000", minHeight: "100vh", fontFamily: NV }}>
        ...
        <div className="max-w-[1200px] mx-auto px-6 text-center">
          ...
          <div style={{ display: "inline-flex", border: BORDER, overflow: "hidden", padding: 3, gap: 3 }}>
            ...
            <button onClick={() => setAnnual(true)} style={{ fontFamily: NV, fontSize: 11, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", padding: "8px 20px", border: "none", cursor: "pointer", background: annual ? "#fff" : "transparent", color: annual ? "#080808" : "rgba(255,255,255,0.5)", transition: "all 0.2s", position: "relative" }}>
              Annual
              <span style={{ position: "absolute", top: -8, right: -4, fontFamily: NV, fontSize: 9, fontWeight: 800, background: "#fff", color: "#080808", padding: "2px 6px", letterSpacing: "0.06em" }}>-20%</span>
            </button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4" style={{ border: BORDER }}>
            {tiers.map((t, i) => {
              ...
              return (
                <div key={t.name} className="p-8 flex flex-col relative" style={{ borderRight: (i + 1) % 4 !== 0 ? BORDER : undefined, borderBottom: BORDER, background: isFeatured ? "rgba(255,255,255,0.06)" : "transparent" }}>
                  {t.badge && (
                    <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", fontFamily: NV, fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", textTransform: "uppercase", background: "#fff", color: "#080808", padding: "4px 16px" }}>
                      {t.badge}
                    </div>
                  )}
                  <div className="flex items-center gap-3 mb-2" style={{ marginTop: t.badge ? 28 : 0 }}>
                    <div style={{ width: 32, height: 32, border: isFeatured ? "0.8px solid rgba(255,255,255,0.4)" : BORDER, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <Icon size={14} style={{ color: isFeatured ? "#fff" : "rgba(255,255,255,0.4)" }} />
                    </div>
                  ...
                  <p style={{ fontFamily: NV, fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: isFeatured ? "#fff" : "rgba(255,255,255,0.25)", marginBottom: 24 }}>{t.eyebrow}</p>

                  <div className="mb-7">
                    <div className="flex items-baseline gap-1">
                      <span style={{ fontFamily: NV, fontWeight: 950, fontSize: "clamp(36px,4vw,48px)", color: isFeatured ? "#fff" : "#fff", letterSpacing: "-0.05em", lineHeight: 1 }}>${price}</span>
                  ...
                  <ul style={{ listStyle: "none", padding: 0, margin: "0 0 24px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
                    {t.features.map((f) => (
                      <li key={f} className="flex gap-2 items-start">
                        <CheckCircle2 size={12} style={{ color: isFeatured ? "#fff" : "rgba(255,255,255,0.25)", flexShrink: 0, marginTop: 2 }} />
                        <span style={{ fontFamily: NV, fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.5 }}>{f}</span>
                      </li>
                    ))}
                  </ul>

                  <a href={t.ctaHref} className="inline-flex items-center justify-center gap-2 hover:opacity-90 transition-opacity" style={{ fontFamily: NV, fontSize: 11, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", background: isFeatured ? "#fff" : "transparent", color: "#fff", border: isFeatured ? "none" : "0.8px solid rgba(255,255,255,0.25)", padding: "10px 22px", textDecoration: "none" }}>
      ...
      <section className="py-16 md:py-20" style={{ borderBottom: BORDER }}>
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="mb-10">
            <p style={{ fontFamily: NV, fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", marginBottom: 12 }}>Compare</p>
              ...
              <thead>
                <tr style={{ borderBottom: BORDER }}>
                  ...
                  {["Starter", "Core", "Prime", "Black"].map((col) => (
                    <th key={col} style={{ textAlign: "center", padding: "16px 16px", fontFamily: NV, fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: col === "Prime" ? "#fff" : "rgba(255,255,255,0.4)" }}>{col}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <>
                    ...
                    {group.features.map((f) => (
                      <tr key={f.name} style={{ borderBottom: BORDER }}>
                        ...
                        <td style={{ textAlign: "center", padding: "14px 16px", background: "rgba(255,255,255,0.04)" }}><CellValue value={f.prime} /></td>
      ...
      <section className="py-16 md:py-20" style={{ borderBottom: BORDER }}>
        <div className="max-w-[800px] mx-auto px-6">
          <div className="text-center mb-14">
            <p style={{ fontFamily: NV, fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", marginBottom: 16 }}>FAQ</p>
