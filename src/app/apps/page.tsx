...
const APP_ICON_DOMAINS: Record<string, string> = {
  ...
  "facebook-shop": "facebook.com",
  "instagram-shopping": "instagram.com",
  "tiktok-shop": "tiktok.com",
  "amazon-channel": "amazon.com",
  "ebay-channel": "ebay.com",
  "google-shopping": "google.com",
  "pinterest-channel": "pinterest.com",
  "etsy-marketplace": "etsy.com",
  "walmart-marketplace": "walmart.com",
  "faire-wholesale": "faire.com",
  "wish-marketplace": "wish.com",
  "mercado-libre": "mercadolibre.com",
  "taobao-channel": "taobao.com",
  dsers: "dsers.com",
  spocket: "spocket.co",
...
const DEVELOPER_ICON_DOMAINS: Record<string, string> = {
  Shopify: "shopify.com",
  Meta: "meta.com",
  Google: "google.com",
  Amazon: "amazon.com",
...
function getAppIconDomain(app: App) {
  return APP_ICON_DOMAINS[app.id] ?? DEVELOPER_ICON_DOMAINS[app.developer]
}

function AppIcon({ app, size = "md" }: { app: App; size?: "md" | "lg" }) {
  const domain = getAppIconDomain(app)
  return <BrandIcon name={app.name} domain={domain} size={size} />
}
    ...
    <span className="inline-flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          ...
          className={
            i < full
              ? "fill-white/55 text-white/55"
              : i === full && half
                ? "fill-white/30 text-white/45"
                : "text-white/30"
          }
        />
      ))}
...
function PricingBadge({ type, label }: { type: PricingType; label: string }) {
  const colors: Record<PricingType, string> = {
    free: "bg-white/[0.03] text-white/55 border-white/[0.12]",
    freemium: "bg-white/[0.03] text-white/55 border-white/[0.12]",
    paid: "bg-white/[0.03] text-white/55 border-white/[0.12]",
    subscription: "bg-white/[0.03] text-white/55 border-white/[0.12]",
  }
  return (
    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-none border ${colors[type]}`}>
      {label}
    </span>
  )
}
...
function AppBadge({ badge }: { badge: Badge }) {
  ...
  const config: Record<string, { label: string; cls: string }> = {
    "staff-pick": { label: "Staff Pick", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
    trending: { label: "Trending", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
    new: { label: "New", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
    popular: { label: "Popular", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
    "built-in": { label: "Built-in", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
    verified: { label: "Verified", cls: "bg-white/[0.03] text-white/55 border-white/[0.12]" },
  }
  const c = config[badge]
  if (!c) return null
  return (
    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-none border ${c.cls}`}>
      {c.label}
    </span>
  )
}
...
function AppCard({ app, view }: { app: App; view: "grid" | "list" }) {
  if (view === "list") {
    return (
      <div className="group flex items-center gap-5 p-4 bg-white/[0.03] border border-white/[0.08] rounded-none hover:border-white/[0.18] hover:bg-black/[0.03] transition-all duration-300">
        <div className="flex-shrink-0 w-12 h-12 rounded-none bg-black/[0.03] border border-white/[0.08] flex items-center justify-center">
          <AppIcon app={app} />
        </div>
        ...
        <div className="flex-shrink-0 flex items-center gap-3">
          ...
          <button className="px-4 py-1.5 text-xs font-medium rounded-none bg-black/[0.04] border border-white/[0.12] text-white hover:bg-[#fff] hover:border-[#fff] hover:text-black transition-all duration-300">
            Install
          </button>
        </div>
      </div>
  ...
  return (
    <div className="group relative flex flex-col p-5 bg-white/[0.03] border border-white/[0.08] rounded-none hover:border-white/[0.18] hover:bg-black/[0.03] transition-all duration-300">
      <div className="flex items-start justify-between mb-4">
        <div className="w-12 h-12 rounded-none bg-black/[0.03] border border-white/[0.08] flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
          <AppIcon app={app} />
        </div>
        <AppBadge badge={app.badge} />
      </div>
      ...
      <div className="flex items-center justify-between">
        <PricingBadge type={app.pricingType} label={app.pricing} />
        <button className="flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-medium rounded-none bg-black/[0.04] border border-white/[0.12] text-white hover:bg-[#fff] hover:border-[#fff] hover:text-black transition-all duration-300 group/btn">
          <Download size={12} />
          Install
        </button>
      </div>
...
function FeaturedCard({ app }: { app: App }) {
  return (
    <div className="group flex-shrink-0 w-[320px] snap-start p-5 bg-white/[0.03] border border-white/[0.08] rounded-none hover:border-white/[0.18] hover:bg-black/[0.03] transition-all duration-300">
      <div className="flex items-start gap-4 mb-4">
        <div className="w-14 h-14 rounded-none bg-black/[0.03] border border-white/[0.08] flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
      ...
      <button className="mt-4 w-full py-2 text-sm font-medium rounded-none bg-black/[0.04] border border-white/[0.12] text-white hover:bg-[#fff] hover:border-[#fff] hover:text-black transition-all duration-300">
        View App
      </button>
    </div>
  )
    ...
    <>
      ...
      <main className="min-h-screen pt-20" style={{ background: "#080808" }}>
        ...
        <section className="relative overflow-hidden border-b border-white/[0.08]">
          <div className="relative max-w-[1400px] mx-auto px-6 lg:px-8 py-14 lg:py-18">
            <div className="max-w-3xl">
              <div className="arabic-row mb-8 border-b border-white/[0.08] pb-4 text-xs uppercase text-white/44">
                <Sparkles size={14} className="text-[#fff]" />
                Powering 13,000+ businesses worldwide <ArabicAccent>متجر التطبيقات</ArabicAccent>
              </div>
              ...
              <div className="relative max-w-2xl mb-8">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-5 py-3 bg-white/[0.02] border border-white/[0.12] rounded-none text-white placeholder:text-white/30 focus:outline-none focus:border-white/[0.2] transition-all duration-200 text-sm"
                />
                {searchQuery && (
                  <button
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 border border-white/[0.08]">
                {[
                  ...
                  <div key={s.label} className="p-4 border-r border-white/[0.08] last:border-r-0">
                    <div className="text-xl md:text-2xl font-bold text-white font-mono">{s.value}</div>
                    <div className="text-xs text-white/44 mt-1 uppercase">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-4">
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide pb-1" style={{ scrollbarWidth: "none" }}>
              <button
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-none text-xs font-medium transition-all duration-300 border ${
                  !selectedCategory
                    ? "bg-[#fff] border-[#fff] text-black"
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-none text-xs font-medium transition-all duration-300 border ${
                    ...
                    <div key={app.id} className="flex items-center gap-5 p-5 hover:bg-white/[0.02] transition-colors" style={{ borderBottom: "0.8px solid rgba(255,255,255,0.08)", borderRight: idx % 2 === 0 ? "0.8px solid rgba(255,255,255,0.08)" : undefined }}>
                      ...
                      <div className="flex-shrink-0 flex items-center gap-3">
                        <div className="flex items-center gap-1">
                          <Star size={11} className="fill-white/55 text-white/55" />
                          <span className="text-xs text-white/44">{app.rating}</span>
                        </div>
                        <PricingBadge type={app.pricingType} label={app.pricing} />
                      </div>
            ...
            <section className="border-b border-white/[0.08]">
              <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-14">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <Zap size={16} className="text-white/55" />
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-14">
            ...
            <div className="flex gap-8">
              ...
              <aside className="hidden lg:block w-56 flex-shrink-0">
                <div className="sticky top-28">
                  ...
                  <nav className="space-y-0.5">
                    <button
                      onClick={() => { setSelectedCategory(null); setVisibleCount(ITEMS_PER_PAGE) }}
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-none text-sm transition-all duration-200 ${
                        !selectedCategory
                          ? "bg-[#fff]/10 text-[#d4d4d4] border border-[#fff]/20"
              ...
              <button
                ...
                className="lg:hidden fixed bottom-6 left-6 z-40 p-3 rounded-none bg-[#fff] text-black "
              >
                <Filter size={20} />
              </button>
                  ...
                  >
                    ...
                    <nav className="space-y-0.5">
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-none text-sm transition-all ${
                          !selectedCategory ? "bg-[#fff]/10 text-[#d4d4d4]" : "text-white/44 hover:text-white"
                        }`}
                      >
                        <span>All Apps</span>
                        <span className="text-xs text-white/30">{ALL_APPS.length}</span>
                ...
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-white/[0.03] border border-white/[0.08] rounded-none text-xs text-white/44 hover:text-white hover:border-white/[0.15] transition-all"
                    >
                      Sort: {sortBy.charAt(0).toUpperCase() + sortBy.slice(1)}
                      <ChevronDown size={13} />
                    </button>
                    {sortOpen && (
                      <div className="absolute top-full left-0 mt-1 w-40 bg-black border border-white/[0.12] rounded-none overflow-hidden z-20 ">
                  ...
                  {/* View toggle */}
                  <div className="flex items-center bg-white/[0.03] border border-white/[0.08] rounded-none overflow-hidden">
                    <button
                ...
                {filteredApps.length === 0 ? (
                  <div className="text-center py-20">
                    ...
                    <button
                      ...
                      className="mt-4 px-5 py-2 text-sm rounded-none bg-black/[0.04] border border-white/[0.12] text-white/44 hover:text-white hover:border-white/[0.18] transition-all"
                    >
                      Clear all filters
                    </button>
                  </div>
                ...
                {visibleCount < filteredApps.length && (
                  <div className="text-center mt-10">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-8 py-3 text-sm font-medium rounded-none bg-black/[0.03] border border-white/[0.12] text-white/70 hover:bg-[#fff] hover:border-[#fff] hover:text-black transition-all duration-300"
                    >
                      Load More Apps
                      <ChevronDown size={16} />
                    </button>
          ...
          <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-20">
            <div className="border border-white/[0.08] bg-white/[0.02] p-8 lg:p-10">
              <div className="grid lg:grid-cols-2 gap-8 items-center">
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-none bg-[#fff]/10 border border-[#fff]/20 text-xs text-[#d4d4d4] mb-6">
                    <Cpu size={12} />
                    Developer Program
                  </div>
                  <h2 className="text-2xl lg:text-3xl font-sans text-white mb-4">
                    Build for <span className="text-white">9Ruby</span>
                  </h2>
                  <p className="text-sm text-white/44 leading-relaxed mb-7">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <a href="#" className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold rounded-none bg-[#fff] text-black hover:bg-white/90 transition-all ">
                      Start Building <ArrowRight size={16} />
                    </a>
                    <a href="#" className="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium rounded-none bg-black/[0.03] border border-white/[0.12] text-white/70 hover:text-white hover:border-white/[0.18] transition-all">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={stat.label} className="p-5 rounded-none bg-white/[0.03] border border-white/[0.08]">
                      <stat.icon size={20} className="text-[#fff] mb-3" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              {[
                ...
                <a
                  key={resource.title}
                  href={resource.href}
                  className="group p-5 rounded-none bg-white/[0.03] border border-white/[0.08] hover:border-white/[0.18] hover:bg-black/[0.03] transition-all duration-300"
                >
                  <resource.icon size={20} className="text-white/30 group-hover:text-[#fff] transition-colors mb-3" />
