...
function PriorityPill({ priority }: { priority: 'high' | 'medium' | 'low' }) {
  const config = {
    high: 'text-white/70 bg-white/10',
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono ${config[priority]}`}>
      <span className={`w-1.5 h-1.5 rounded-full ${
        priority === 'high' ? 'bg-white/60' : priority === 'medium' ? 'bg-amber-400' : 'bg-white/30'
      }`} />
      {priority}
    </span>
  )
...
function TaskStatusBadge({ status }: { status: 'done' | 'active' | 'pending' }) {
  const config = {
    ...
    active: { text: 'text-[var(--accent)]', bg: 'bg-[var(--accent)]/10', icon: Zap },
              ...
              <form onSubmit={handleAddMember} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white border border-black/[0.08] text-[#111] text-sm placeholder:text-[#B8B8B0] focus:outline-none focus:border-black/[0.15] transition-colors"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-4 py-2.5 rounded-xl bg-white border border-black/[0.08] text-[#111] text-sm appearance-none focus:outline-none focus:border-black/[0.15] transition-colors cursor-pointer"
                    >
                      {roles.map((r) => (
